...
class Model:
    ...
    def __init__(self):
        ...
        self.cur=None
        try:
            self.conn= connect(host="localhost",user="mouzikka",password="music",database="pythonproject")
    ...
    def close_db_connection(self):
        ...
        if self.conn is not None:
            self.conn.close()
            print("Disconnected from DB successfully")
    ...
    def search_song_in_favourites(self,song_name):
        self.cur.execute("select song_name from myfavourites where song_name=%s",(song_name,))
    ...
    def add_to_favourites(self,song_name,song_path):
        ...
        self.cur.execute("insert into myfavourites values(%s,%s,%s)",(next_song_id,song_name,song_path))
        self.conn.commit()
    ...
    def remove_song_favourites(self,song_name):
        self.cur.execute("Delete from myfavourites where song_name=%s",(song_name))
        n=self.cur.rowcount
        if n!=0:
